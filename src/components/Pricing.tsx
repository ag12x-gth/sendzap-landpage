...
export const Pricing: React.FC = () => {
  ...
  // Checkout Modal State (Pagar.me v5)
  ...
  const [customerNeighborhood, setCustomerNeighborhood] = useState<string>('');
  const [customerCity, setCustomerCity] = useState<string>('');
  const [customerState, setCustomerState] = useState<string>('');
  
  // Dados de Pagamento
  const [paymentMethod, setPaymentMethod] = useState<'credit_card' | 'pix' | 'google_pay' | 'apple_pay'>('credit_card');
  ...
  const generatePagarmeToken = async (walletPayload?: any, walletType?: 'google_pay' | 'apple_pay') => {
    ...
    // Se não existir, avisa (isso é crítico em prod)
    ...
    let payload: any = { type: 'card' };

    if (walletPayload && walletType === 'google_pay') {
      // Parse Google Pay token format
      const tokenStr = walletPayload.paymentMethodData.tokenizationData.token;
      const tokenObj = JSON.parse(tokenStr);
      const signedMessage = JSON.parse(tokenObj.signedMessage);
      
      payload.card = {
        google_pay: {
          version: tokenObj.protocolVersion,
          data: signedMessage.encryptedMessage,
          signature: tokenObj.signature,
          ephemeral_public_key: signedMessage.ephemeralPublicKey
        }
      };
    } else if (walletPayload && walletType === 'apple_pay') {
      const appleToken = walletPayload.token.paymentData;
      payload.card = {
        apple_pay: {
          ...
          header: {
            ephemeral_public_key: appleToken.header.ephemeralPublicKey,
            public_key_hash: appleToken.header.publicKeyHash,
            transaction_id: appleToken.header.transactionId
          }
        }
      };
    } else {
      payload.card = {
        ...
        holder_document: customerCpf.replace(/\D/g, ''),
    ...
    if (!response.ok) {
      ...
      throw new Error('Erro ao validar cartão de crédito. Verifique os dados inseridos.');
    }
  ...
  const handleConfirmCheckout = async (e: React.FormEvent) => {
    ...
    const docLength = customerCpf ? customerCpf.replace(/\D/g, '').length : 0;
    if (docLength !== 11 && docLength !== 14) {
      setCheckoutError('Por favor, insira um CPF (11 dígitos) ou CNPJ (14 dígitos) válido.');
      return;
    }
    ...
    try {
      ...
      if (paymentMethod === 'credit_card') {
        // Passo 1: Tokenizar cartão no FrontEnd
        cardToken = await generatePagarmeToken();
      }

      // Passo 2: Enviar ao Backend
      const response = await fetch('https://agente-funil-checkout-production.up.railway.app/api/v1/checkout/transparente', {
        method: 'POST',
        headers: {
      ...
      if (!response.ok) {
        throw new Error(data.error || 'Erro ao processar o pagamento. Tente novamente.');
      }
      ...
      if (paymentMethod === 'pix') {
        if (data.pix_qr_code_url) {
          ...
          setPixCopiaECola(data.pix_qr_code || '');
          // Opcional: implementar um polling para checar quando foi pago
          // Polling para checar o status e redirecionar apenas quando o Pix for pago
          const checkStatus = setInterval(async () => {
            try {
              const res = await fetch(`https://agente-funil-checkout-production.up.railway.app/api/v1/checkout/status/${data.order_id}`);
              if (res.ok) {
                const statusData = await res.json();
                if (statusData.status === 'paid') {
                  clearInterval(checkStatus);
                  setCheckoutSuccess(true);
                  // Redirecionamento após alguns segundos do pagamento confirmado
                  setTimeout(() => {
          ...
          throw new Error('QR Code do Pix não retornado pela Pagar.me.');
        }
      } else {
        // Cartão de Crédito aprovado!
        if (data.status === 'paid') {
          // Disparar o Pixel (embora o script do header faça automático em /obrigado)
          if ((window as any).fbq) {
            (window as any).fbq('track', 'Purchase', { currency: 'BRL', value: selectedPlan.price.replace(',', '.') });
          }
          setCheckoutSuccess(true);
          ...
          // Pode cair em 'pending' se tiver antifraude ou 'failed'
          if (data.status === 'failed') {
            throw new Error('Pagamento recusado pelo emissor do cartão.');
          } else {
            setCheckoutError('Pagamento em análise. Você receberá um e-mail com a confirmação em breve.');
            setTimeout(() => {
              window.location.href = '/obrigado.html';
            }, 7000);
          }
  ...
  const handleWalletCheckout = async (walletPayload: any, walletType: 'google_pay' | 'apple_pay') => {
    setIsSubmittingCheckout(true);
    setCheckoutError('');
    ...
    try {
      // Passo 1: Tokenizar carteira no FrontEnd via API da Pagar.me
      const cardToken = await generatePagarmeToken(walletPayload, walletType);
      ...
      const response = await fetch('https://agente-funil-checkout-production.up.railway.app/api/v1/checkout/transparente', {
        ...
        body: JSON.stringify({
          product_id: productId,
          customer_email: leadEmail || 'googlepay@example.com', // fallback
          customer_name: customerName || 'Usuário Carteira Digital',
          customer_cpf: customerCpf.replace(/\D/g, '') || '00000000000', // needs real CPF
          customer_phone: customerPhone,
          customer_zipcode: customerZipCode || '01001000',
          customer_street: customerStreet || 'Rua Nao Informada',
          customer_number: customerNumber || '1',
          customer_complement: customerComplement,
          customer_neighborhood: customerNeighborhood || 'Centro',
          customer_city: customerCity || 'Sao Paulo',
          customer_state: customerState || 'SP',
          payment_method: 'credit_card', // backend accepts it as credit_card
          card_token: cardToken,
          installments: 1
        })
      });
      ...
      if (!response.ok) {
        throw new Error(data.error || 'Erro ao processar o pagamento pela carteira.');
      }
      ...
      if (data.status === 'paid') {
        ...
        if (data.status === 'failed') {
          throw new Error('Pagamento recusado pela carteira.');
        } else {
          setCheckoutError('Pagamento em análise. Você receberá um e-mail com a confirmação em breve.');
          setTimeout(() => {
            window.location.href = '/obrigado.html';
  ...
  const googlePaymentRequest = React.useMemo(() => ({
    apiVersion: 2,
    apiVersionMinor: 0,
    allowedPaymentMethods: [
      {
        ...
        tokenizationSpecification: {
          ...
          parameters: {
            gateway: 'pagarme',
            gatewayMerchantId: 'sendzap_store'
          },
        },
      },
    ],
    merchantInfo: {
      ...
      merchantName: 'SendZap',
    },
    transactionInfo: {
      totalPriceStatus: 'FINAL',
      totalPriceLabel: 'Total',
      totalPrice: selectedPlan?.price ? selectedPlan.price.replace(/\./g, '').replace(',', '.') : '0.00',
      currencyCode: 'BRL',
      countryCode: 'BR',
    },
  }), [selectedPlan?.price]);
          ...
          >
            ...
            {checkoutSuccess ? (
              ...
              <div style={{ textAlign: 'center' }}>
                <h4 style={{ fontSize: '16px', fontWeight: '700', color: '#FFFFFF', marginBottom: '16px' }}>Escaneie o QR Code abaixo para pagar</h4>
                <img src={pixQrCodeUrl} alt="QR Code Pix" style={{ width: '200px', height: '200px', borderRadius: '10px', marginBottom: '16px', background: '#FFF', padding: '10px' }} />
                
                {pixCopiaECola && (
                  <div style={{ marginBottom: '16px', display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
                    <p style={{ color: 'rgba(255, 255, 255, 0.7)', fontSize: '13px', marginBottom: '8px' }}>Ou copie o código Pix (Copia e Cola):</p>
                    <div style={{ display: 'flex', width: '100%', maxWidth: '300px', background: 'rgba(255, 255, 255, 0.05)', border: '1px solid #333', borderRadius: '6px', overflow: 'hidden' }}>
                      <input 
                        type="text" 
                        value={pixCopiaECola} 
                        readOnly 
                        style={{ flex: 1, background: 'transparent', border: 'none', color: '#FFF', padding: '8px 12px', fontSize: '12px', outline: 'none' }} 
                      />
                      <button 
                        type="button"
                        onClick={() => {
                        ...
                        style={{ background: '#00CB6F', color: '#000', border: 'none', padding: '0 12px', fontWeight: 'bold', cursor: 'pointer', fontSize: '12px' }}
                      >
                        COPIAR
                      </button>
                    </div>
                ...
                <p style={{ color: 'rgba(255, 255, 255, 0.7)', fontSize: '13px' }}>Após o pagamento, aguarde alguns segundos.</p>
              </div>
            ) : (
              <form onSubmit={handleConfirmCheckout} style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
                ...
                <div style={{ display: 'flex', gap: '10px', marginBottom: '8px', flexWrap: 'wrap' }}>
                  <button
                    ...
                    style={{
                      ...
                      padding: '10px',
                      borderRadius: '8px',
                      border: paymentMethod === 'credit_card' ? '1px solid #00CB6F' : '1px solid #262626',
                      background: paymentMethod === 'credit_card' ? 'rgba(0, 203, 111, 0.1)' : '#0D0D0D',
                      color: paymentMethod === 'credit_card' ? '#00CB6F' : '#FFF',
                      fontWeight: '700',
                      cursor: 'pointer'
                    }}
                  >
                  ...
                  <button
                    ...
                    style={{
                      ...
                      padding: '10px',
                      borderRadius: '8px',
                      border: paymentMethod === 'pix' ? '1px solid #00CB6F' : '1px solid #262626',
                      background: paymentMethod === 'pix' ? 'rgba(0, 203, 111, 0.1)' : '#0D0D0D',
                      color: paymentMethod === 'pix' ? '#00CB6F' : '#FFF',
                      fontWeight: '700',
                      cursor: 'pointer'
                    }}
                  >
                  ...
                  <button
                    ...
                    style={{
                      ...
                      padding: '10px',
                      borderRadius: '8px',
                      border: paymentMethod === 'google_pay' ? '1px solid #00CB6F' : '1px solid #262626',
                      background: paymentMethod === 'google_pay' ? 'rgba(0, 203, 111, 0.1)' : '#0D0D0D',
                      color: paymentMethod === 'google_pay' ? '#00CB6F' : '#FFF',
                      fontWeight: '700',
                      cursor: 'pointer'
                    }}
                  >
                  ...
                  {typeof window !== 'undefined' && (window as any).ApplePaySession && (
                    <button
                      ...
                      style={{
                        ...
                        padding: '10px',
                        borderRadius: '8px',
                        border: paymentMethod === 'apple_pay' ? '1px solid #00CB6F' : '1px solid #262626',
                        background: paymentMethod === 'apple_pay' ? 'rgba(0, 203, 111, 0.1)' : '#0D0D0D',
                        color: paymentMethod === 'apple_pay' ? '#00CB6F' : '#FFF',
                        fontWeight: '700',
                        cursor: 'pointer'
                      }}
                    >
                ...
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '12px' }}>
                  ...
                  <div>
                    ...
                    <input type="text" required placeholder="000.000.000-00 ou 00.000.000/0000-00" value={customerCpf} onChange={(e) => setCustomerCpf(e.target.value)}
                      style={{ width: '100%', padding: '10px', borderRadius: '8px', background: '#0D0D0D', border: '1px solid #262626', color: '#FFF', fontSize: '14px', outline: 'none' }} />
                  </div>
                  <div>
                    <label style={{ display: 'block', fontSize: '12px', fontWeight: '700', marginBottom: '4px', color: 'rgba(255, 255, 255, 0.9)' }}>
                    ...
                    <input type="tel" required placeholder="(11) 99999-9999" value={customerPhone} onChange={(e) => {
                ...
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '12px' }}>
                  <div>
                    ...
                    <input type="text" required placeholder="00000-000" value={customerZipCode} onChange={handleZipCodeChange}
                      style={{ width: '100%', padding: '10px', borderRadius: '8px', background: '#0D0D0D', border: '1px solid #262626', color: '#FFF', fontSize: '14px', outline: 'none' }} />
                  </div>
                  <div>
                    <label style={{ display: 'block', fontSize: '12px', fontWeight: '700', marginBottom: '4px', color: 'rgba(255, 255, 255, 0.9)' }}>
                    ...
                    <input type="text" required placeholder="Sua Rua" value={customerStreet} onChange={(e) => setCustomerStreet(e.target.value)}
                      style={{ width: '100%', padding: '10px', borderRadius: '8px', background: '#0D0D0D', border: '1px solid #262626', color: '#FFF', fontSize: '14px', outline: 'none' }} />
                  </div>
                </div>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 2fr', gap: '12px' }}>
                  <div>
                    ...
                    <input type="text" required placeholder="123" value={customerNumber} onChange={(e) => setCustomerNumber(e.target.value)}
                      style={{ width: '100%', padding: '10px', borderRadius: '8px', background: '#0D0D0D', border: '1px solid #262626', color: '#FFF', fontSize: '14px', outline: 'none' }} />
                  </div>
                  <div>
                    <label style={{ display: 'block', fontSize: '12px', fontWeight: '700', marginBottom: '4px', color: 'rgba(255, 255, 255, 0.9)' }}>
                    ...
                    <input type="text" placeholder="Apto 42" value={customerComplement} onChange={(e) => setCustomerComplement(e.target.value)}
                      style={{ width: '100%', padding: '10px', borderRadius: '8px', background: '#0D0D0D', border: '1px solid #262626', color: '#FFF', fontSize: '14px', outline: 'none' }} />
                  </div>
                </div>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: '12px' }}>
                  <div>
                    ...
                    <input type="text" required placeholder="Bairro" value={customerNeighborhood} onChange={(e) => setCustomerNeighborhood(e.target.value)}
                      style={{ width: '100%', padding: '10px', borderRadius: '8px', background: '#0D0D0D', border: '1px solid #262626', color: '#FFF', fontSize: '14px', outline: 'none' }} />
                  </div>
                  <div>
                    <label style={{ display: 'block', fontSize: '12px', fontWeight: '700', marginBottom: '4px', color: 'rgba(255, 255, 255, 0.9)' }}>
                    ...
                    <input type="text" required placeholder="Cidade" value={customerCity} onChange={(e) => setCustomerCity(e.target.value)}
                      style={{ width: '100%', padding: '10px', borderRadius: '8px', background: '#0D0D0D', border: '1px solid #262626', color: '#FFF', fontSize: '14px', outline: 'none' }} />
                  </div>
                  <div>
                    <label style={{ display: 'block', fontSize: '12px', fontWeight: '700', marginBottom: '4px', color: 'rgba(255, 255, 255, 0.9)' }}>
                    ...
                    <input type="text" required placeholder="SP" maxLength={2} value={customerState} onChange={(e) => setCustomerState(e.target.value.toUpperCase())}
                      style={{ width: '100%', padding: '10px', borderRadius: '8px', background: '#0D0D0D', border: '1px solid #262626', color: '#FFF', fontSize: '14px', outline: 'none', textTransform: 'uppercase' }} />
                  </div>
                </div>

                {paymentMethod === 'credit_card' && (
                  <div style={{ display: 'flex', flexDirection: 'column', gap: '12px', marginTop: '4px', borderTop: '1px solid #262626', paddingTop: '16px' }}>
                    ...
                    <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '12px' }}>
                      <div>
                        ...
                        <input type="text" required placeholder="JOAO SILVA" value={cardHolderName} onChange={(e) => setCardHolderName(e.target.value)}
                          style={{ width: '100%', padding: '10px', borderRadius: '8px', background: '#0D0D0D', border: '1px solid #262626', color: '#FFF', fontSize: '14px', outline: 'none', textTransform: 'uppercase' }} />
                      </div>
                      <div style={{ display: 'flex', gap: '8px' }}>
                        <div style={{ flex: 1 }}>
                          <label style={{ display: 'block', fontSize: '12px', fontWeight: '700', marginBottom: '4px', color: 'rgba(255, 255, 255, 0.9)' }}>Mês</label>
                          <input type="text" required placeholder="MM" maxLength={2} value={cardExpMonth} onChange={(e) => setCardExpMonth(e.target.value)}
                            style={{ width: '100%', padding: '10px', borderRadius: '8px', background: '#0D0D0D', border: '1px solid #262626', color: '#FFF', fontSize: '14px', outline: 'none' }} />
                        </div>
                        <div style={{ flex: 1 }}>
                          <label style={{ display: 'block', fontSize: '12px', fontWeight: '700', marginBottom: '4px', color: 'rgba(255, 255, 255, 0.9)' }}>Ano</label>
                          <input type="text" required placeholder="AA" maxLength={2} value={cardExpYear} onChange={(e) => setCardExpYear(e.target.value)}
                            style={{ width: '100%', padding: '10px', borderRadius: '8px', background: '#0D0D0D', border: '1px solid #262626', color: '#FFF', fontSize: '14px', outline: 'none' }} />
                        </div>
                        <div style={{ flex: 1 }}>
                          <label style={{ display: 'block', fontSize: '12px', fontWeight: '700', marginBottom: '4px', color: 'rgba(255, 255, 255, 0.9)' }}>CVV</label>
                          <input type="text" required placeholder="123" maxLength={4} value={cardCvv} onChange={(e) => setCardCvv(e.target.value)}
                            style={{ width: '100%', padding: '10px', borderRadius: '8px', background: '#0D0D0D', border: '1px solid #262626', color: '#FFF', fontSize: '14px', outline: 'none' }} />
                        </div>
                      </div>
                    </div>
                ...
                {paymentMethod === 'google_pay' ? (
                  <div style={{ marginTop: '8px', width: '100%', display: 'flex', justifyContent: 'center' }}>
                    <GooglePayButton
                      environment="TEST" // Change to PRODUCTION when approved
                      paymentRequest={googlePaymentRequest as any}
                      onLoadPaymentData={(paymentRequest) => {
                        handleWalletCheckout(paymentRequest, 'google_pay');
                      }}
                      onError={(err) => {
                        console.error('Google Pay Error:', err);
                        setCheckoutError('Erro ao iniciar o Google Pay.');
                      }}
                      buttonColor="default"
                      buttonType="pay"
                      style={{ width: '100%' }}
                  ...
                  <button
                    ...
                    onClick={() => {
                      ...
                      const session = new ApplePaySession(3, {
                        ...
                        total: {
                          label: 'SendZap',
                          amount: selectedPlan?.price ? selectedPlan.price.replace(/\./g, '').replace(',', '.') : '0.00'
                        }
                      });
                      session.onvalidatemerchant = async (event: any) => {
                         try {
                           const res = await fetch('https://agente-funil-checkout-production.up.railway.app/api/v1/checkout/apple-pay/validate', {
                             method: 'POST',
                             headers: { 'Content-Type': 'application/json' },
                           ...
                           if (!res.ok) {
                             ...
                             setCheckoutError(errData.error || "Erro ao validar o merchant no Apple Pay.");
                             session.abort();
                             return;
                           }
                           ...
                           setCheckoutError("Erro de conexão ao validar o Apple Pay.");
                           session.abort();
                         }
                      };
                      session.onpaymentauthorized = (event: any) => {
                         handleWalletCheckout(event.payment, 'apple_pay');
                  ...
                  >
                    {isSubmittingCheckout ? 'Processando Pagamento...' : 'Concluir Pagamento Seguro 🔒'}
                  </button>
                )}
              </form>
            )}
