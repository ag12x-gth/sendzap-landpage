const fs = require('fs');
let content = fs.readFileSync('e:/Projetos-Antigrav/agente-funil/sendzap-landing/src/components/Pricing.tsx', 'utf8');
...
// 3. Replace handleConfirmCheckout
const handleStartStr = '  const generatePagarmeToken = async (walletPayload?: any, walletType?: \'google_pay\' | \'apple_pay\') => {';
const handleEndStr = '  // Visit Count & Expiration Tracker (localStorage)';
...
const zipEndIdx = content.indexOf(zipEndStr, zipStartIdx);
if (zipStartIdx !== -1 && zipEndIdx !== -1) {
  content = content.substring(0, zipStartIdx) + content.substring(zipEndIdx);
}

// Remove the import of GooglePayButton if it is there
content = content.replace('import GooglePayButton from \'@google-pay/button-react\';', '');

fs.writeFileSync('e:/Projetos-Antigrav/agente-funil/sendzap-landing/src/components/PricingPagarme.tsx', content);
